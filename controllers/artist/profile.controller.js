...
exports.updateUserDetails = async (req, res) => {
  try {
    const { _id } = req.user;
    User.findByIdAndUpdate(_id, req.body).then((details) => {
      if (details) return res.status(200).send({ error: true, message: "User not exits" });
      return res.status(400).send({ error: false, message: "User updated" });
    });
  } catch (error) {
...
exports.addKyc = async (req, res) => {
  try {
    ...
    Kyc.create({
      ...
      await User.findByIdAndUpdate(_id, { status: "PENDING_APPROVAL" })
      res.send({
        error: false,
...
exports.getArtistById = async (req, res) => {
  ...
  //   if (profileDetails) {
  //     return res.status(200).send({ error: false, data: profileDetails });
  //   }
  //   return res.status(400).send({ error: true, message: "Details Not found" });
  // } catch (error) {
  //   res.send({ error: true, message: error.message });
  // }
  ...
  try {
    ...
    return res.status(200).send({ error: false, data: responseData });
  } catch (error) {
    res.status(500).send({ error: true, message: error.message });
  }
};
...
exports.getAllArtists = async (req, res) => {
  try {
    const profileDetails = await User.find({user_role: USER_ROLE_ARTIST});
    if (profileDetails) {
      return res.status(200).send({ error: false, data: profileDetails });
    }
    return res.status(400).send({ error: true, message: "Details Not found" });
  ...
  try {
    ...
    if (packageDetails) {
      await Package.findOneAndUpdate(
        ...
        res.status(200).send({ error: false, message: "Package updated successfully" });
      }).catch(e => {
        res.status(500).send({ error: true, message: error.message });
      });
      return
    ...
    await Package.create({ ...req?.body, artist: _id }).then(() => {
      res.send({ error: false, message: "Package created successfully" });
    }).catch(e => {
      res.status(500).send({ error: true, message: error.message });
    });;
  } catch (error) {
...
exports.getAllPackageByArtist = async (req, res) => {
  try {
    const { _id } = req.user;
    const package = await Package.find({
      artist: _id,
    });
    res.status(200).send({ error: false, data: package });
  } catch (error) {
    res.status(500).send({ error: true, message: error.message });
  }
};

exports.getAllPackageByArtistById = async (req, res) => {
  try {
    const { id } = req.params;
    const package = await Package.find({
      artist: id,
