...
exports.addPost = async (req, res) => {
  try {
    const { caption, post_on_portfolio, post_on_feed, image } = req.body;
...
exports.deletePost = async (req, res) => {
  try {
    ...
    await Post.findByIdAndDelete(id).then((post) => {
      console.log(post)
      if(post){
        res.status(200).send({ error: false, message: "Post deleted."});
      }else{
 ...
 * Type : GET
 * Route : /artist/discover/post/get-one/:id
 */
exports.getPostById = async (req, res) => {
  try {
    ...
    if (!postId) {
      return res.status(400).send({ error: true, message: "Post ID is required" });
    }
    ...
    let reactions = [];
    let reactionTypes = ["love", "surprise", "laugh", "happy"];

    for (let i = 0; i < reactionTypes.length; i++) {
      reactions.push({
        id: i,
        img: i + 6,
        isclick: post?.reactions[reactionTypes[i]]?.includes(userId),
        count: post?.reactions[reactionTypes[i]]?.length || 0,
        name: reactionTypes[i],
      });
    }
 ...
 * Get All Posts
 * Type : GET
 * Route : /artist/discover/post/get-myposts
 */
...
exports.bookmarkPost = async (req, res) => {
  try {
    ...
    if (!bookmark) {
      ...
      let updatedBookmark = await Bookmark.findByIdAndDelete(bookmark._id);
      if(updatedBookmark) return res.status(200).send({ error: false, message: "Bookmark removed." });
      return res.status(404).send({ error: false, message: "Bookmark not found." });
    ...
    return res.send({ error: false, message: "Post Bookmarked." });
  } catch (error) {
    res.send({ error: true, message: error.message });
  }
};
